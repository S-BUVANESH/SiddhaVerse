...
def run_vector_benchmarks():
    ...
    print("Executing Vector-Only Benchmarks...")
    ...
    for q in queries:
        ...
        # Warmup and stabilize latency measurements
        run_latencies = []
        retrieved_ids = []
        
        for i in range(15):  # 15 runs to get robust statistics
            t0 = time.perf_counter()
            response = client.get(f"/api/v1/search?q={query_text}&mode=vector")
    ...
    # Generate vector_benchmark_results.json
    results_json = {
    ...
    with open(r"d:\Siddha_Wisdom\vector_benchmark_results.json", "w", encoding="utf-8") as f:
        json.dump(results_json, f, indent=2, ensure_ascii=False)
        
    # Generate vector_benchmark_results.md
    md_content = f"""# Vector Retrieval Benchmark Results — SiddhaVerse
...
"""
    ...
    with open(r"d:\Siddha_Wisdom\vector_benchmark_results.md", "w", encoding="utf-8") as f:
        f.write(md_content)
        
    print("Wrote vector_benchmark_results.json and vector_benchmark_results.md")
